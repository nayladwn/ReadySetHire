...
//#region node_modules/react-speech-recognition/dist/cc-BU0zEyYq.js
...
//#endregion
//#region node_modules/react-speech-recognition/dist/index.js
...
var isNative = (SpeechRecognition) => SpeechRecognition === NativeSpeechRecognition;
...
var RecognitionManager = class {
	...
	onRecognitionDisconnect() {
		...
		else if (this.recognition) if (this.recognition.continuous) this.startListening({ continuous: this.recognition.continuous });
	...
	updateTranscript({ results, resultIndex }) {
		...
		for (let i = currentIndex; i < results.length; ++i) if (results[i].isFinal && (!isAndroid() || results[i][0].confidence > 0)) this.updateFinalTranscript(results[i][0].transcript);
		else this.interimTranscript = concatTranscripts(this.interimTranscript, results[i][0].transcript);
	...
	startListening() {
		return /* @__PURE__ */ _async_to_generator(function* ({ continuous = false, language } = {}) {
			...
			if (isContinuousChanged || isLanguageChanged) {
				if (this.listening) yield this.stopListening();
			...
			if (!this.listening) {
				...
				try {
					...
					if (!(e instanceof DOMException)) this.emitMicrophoneAvailabilityChange(false);
				}
			}
	...
	constructor(SpeechRecognition) {
		...
		if (isAndroid()) this.updateFinalTranscript = (0, import_lodash_debounce.default)(this.updateFinalTranscript, 250, { leading: true });
	}
};
...
var transcriptReducer = (state, action) => {
	switch (action.type) {
		case CLEAR_TRANSCRIPT: return {
			interimTranscript: "",
			finalTranscript: ""
		};
		case APPEND_TRANSCRIPT: return {
...
var useSpeechRecognition = ({ transcribing = true, clearTranscriptOnListen = true, commands = [] } = {}) => {
	const [recognitionManager] = (0, import_react.useState)(SpeechRecognition.getRecognitionManager());
	...
	const [listening, setListening] = (0, import_react.useState)(recognitionManager.listening);
	const [isMicrophoneAvailable, setMicrophoneAvailable] = (0, import_react.useState)(recognitionManager.isMicrophoneAvailable);
	...
	const testFuzzyMatch = (command, input, fuzzyMatchingThreshold) => {
		const commandWithoutSpecials = (typeof command === "object" ? command.toString() : command).replace(/[&/\\#,+()!$~%.'":*?<>{}]/g, "").replace(/  +/g, " ").trim();
	...
	const testMatch = (command, input) => {
		const result = commandToRegExp(command).exec(input);
		if (result) return {
			command,
	...
	const matchCommands = (0, import_react.useCallback)((newInterimTranscript, newFinalTranscript) => {
		commandsRef.current.forEach(({ command, callback, matchInterim = false, isFuzzyMatch = false, fuzzyMatchingThreshold = .8, bestMatchOnly = false }) => {
			const input = !newFinalTranscript && matchInterim ? newInterimTranscript.trim() : newFinalTranscript.trim();
			const results = (Array.isArray(command) ? command : [command]).map((subcommand) => {
	...
	const handleTranscriptChange = (0, import_react.useCallback)((newInterimTranscript, newFinalTranscript) => {
		if (transcribing) dispatch(appendTranscript(newInterimTranscript, newFinalTranscript));
...
var SpeechRecognition = {
	...
	applyPolyfill: (PolyfillSpeechRecognition) => {
		...
		else recognitionManager = new RecognitionManager(PolyfillSpeechRecognition);
		const browserSupportsPolyfill = !!PolyfillSpeechRecognition && browserSupportsPolyfills();
		_browserSupportsSpeechRecognition = browserSupportsPolyfill;
		_browserSupportsContinuousListening = browserSupportsPolyfill;
	},
	removePolyfill: () => {
		if (recognitionManager) recognitionManager.setSpeechRecognition(NativeSpeechRecognition);
		else recognitionManager = new RecognitionManager(NativeSpeechRecognition);
	...
	getRecognitionManager: () => {
		if (!recognitionManager) recognitionManager = new RecognitionManager(NativeSpeechRecognition);
		return recognitionManager;
	},
	getRecognition: () => {
		return SpeechRecognition.getRecognitionManager().getRecognition();
	},
	startListening: ({ continuous, language } = {}) => /* @__PURE__ */ _async_to_generator(function* () {
		yield SpeechRecognition.getRecognitionManager().startListening({
			continuous,
			language
		});
	})(),
	stopListening: () => /* @__PURE__ */ _async_to_generator(function* () {
		yield SpeechRecognition.getRecognitionManager().stopListening();
	})(),
	abortListening: () => /* @__PURE__ */ _async_to_generator(function* () {
		yield SpeechRecognition.getRecognitionManager().abortListening();
	})(),
